...
export default class UserController {
  ...
  update = async (id: number, request: any, response: any) => {
    try {
      const product = await this.productService.update(id, request.body);
      response.status(201).send(product);
    } catch (error: any) {
      response.status(400).send({ error: error.toString() });
    }
  ...
  delete = async (request: any, response: any) => {
    try {
      console.log(request);
      await this.productService.delete(request);
  ...
  findById = async (request: any, response: any) => {
    try {
      const product = await this.productService.findById(request);
      response.status(201).send(product);
    } catch (error: any) {
      response.status(404).send({ error: error.toString() });
    }
  ...
  findSales = async (request: any, response: any) => {
    try {
      const product = await this.productService.findSales();
      response.status(201).send(product);
    } catch (error: any) {
      response.status(404).send({ error: error.toString() });
    }
